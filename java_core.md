...
-- METHOD OVERLOADING trong JAVA có nghĩa là có hai hoặc nhiều phương thức (hoặc hàm) trong một lớp có cùng tên nhưng
khác nhau về đối số (hoặc tham số). Điều này có thể thực hiện bằng cách sử dụng số lượng đối số khác nhau hoặc
kiểu dữ liệu của các đối số khác nhau.
...
-- Sử dụng với phương thức: Khi một phương thức được khai báo là final, phương thức này không thể bị ghi đè
